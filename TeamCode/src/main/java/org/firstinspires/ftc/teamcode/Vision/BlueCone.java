...
public class BlueCone implements VisionProcessor, CameraStreamSource {


    //color data, using HSV colorspace, H=0-180, S=0-255, V=0-255
    public static int max_H = 175;
    public static int max_S = 275;
...
//this is not used, can be used to draw on image but not really that useful, easier to just do in the main area
    ...
    public double getRectPos() {
        double rectPos = 0;
        if (largestRect != null) {
            if (getCenter() < IMG_WIDTH / 3) {
                rectPos = 1;
            } else if (getCenter() > IMG_WIDTH / 3 && getCenter() < IMG_WIDTH - (IMG_WIDTH / 3)) {
                rectPos = 2;
            } else if (getCenter() > IMG_WIDTH - (IMG_WIDTH / 3)) {
                rectPos = 3;
            }
        }
        return (rectPos);
